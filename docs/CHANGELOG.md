...
### Fixed (v0.3.3 让 `--ocr tesseract` 真正可用)

Tesseract 是**字符级**引擎：实测一页返回 1337 个框（中位宽仅 23px），
按框逐个翻译时译文根本塞不进单字框，实测「262 处放不下 / 243 处没写上」。

- `modules/ocr_engine.py` 新增 `merge_char_boxes()`：按垂直重叠分行、
  行内按间距合并成整行；若两框之间存在贯穿整行的深色竖线（表格框线）
  则不合并，避免把相邻单元格拼成一句。
- 引擎新增 `granularity` 属性（EasyOCR=`line`，Tesseract=`char`），
  `core/pdf_translator.py` 仅在字符级引擎上做合并。

效果（单页扫描件）：区域数 1337 → 275，`放不下` 262 → **4**，耗时 9.8s → **2.6s**。
EasyOCR 路径不受影响（回归验证：空白色块 0 / 底色不符 0）。

### Docs

- 重写 `README.md`：整理为命令速查，补充参数表、`.env` 开关与性能数据。
